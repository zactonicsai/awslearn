...
variable "lambda_runtime" {
  ...
  type = string
  # Use Python 3.14 because AWS Lambda currently supports it as a managed Python runtime.
  default = "python3.14"
}
...
variable "allowed_cors_origins" {
  # Explain that CORS controls what browser websites can call this API.
  description = "Browser origins allowed by CORS; use [\"*\"] for a simple lab."
  # Set the value type to list of strings because there can be many origins.
  type = list(string)
  # Allow all origins for a simple test API.
  default = ["*"]
}
